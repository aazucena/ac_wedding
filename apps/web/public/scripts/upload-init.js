...
if (identityModal) {
  identityModal.querySelectorAll('.identity-btn[data-name]').forEach(btn => {
    btn.addEventListener('click', () => confirmIdentity(btn.dataset.name, btn.dataset.id, btn.dataset.token || null));
  });
...
async function submitTableVerify() {
  ...
  try {
    var res  = await fetch('/api/game/verify', {
      ...
      body: JSON.stringify({ guestId: pendingGuest.id, tableNumber: Number(tableNumber) }),
    });
...
if (tableNumberInput) {
  tableNumberInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') { e.preventDefault(); submitTableVerify(); }
  });
}
...
// ── Persistence ───────────────────────────────────────────────────────────────
var SK = { name: 'gup-name', squares: 'gup-squares', confirmed: 'gup-confirmed', submitted: 'gup-submitted', guestId: 'gup-guestid', token: 'gup-token' };
var photoKey = function(n) { return 'gup-photo-' + n; };

function persist() {
  try {
    sessionStorage.setItem(SK.name,      nameInput.value.trim());
    sessionStorage.setItem(SK.confirmed, nameConfirmed && !nameConfirmed.classList.contains('hidden') ? '1' : '');
    sessionStorage.setItem(SK.guestId,   confirmedGuestId || '');
    sessionStorage.setItem(SK.token,     confirmedToken   || '');
    sessionStorage.setItem(SK.squares,   JSON.stringify([...squareDataURLs.keys()]));
    sessionStorage.setItem(SK.submitted, JSON.stringify([...submittedSquares]));
    for (var [n, url] of squareDataURLs) sessionStorage.setItem(photoKey(n), url);
  } catch (_) {}
}

function clearStorage() {
  try {
    [SK.name, SK.confirmed, SK.squares, SK.submitted, SK.guestId, SK.token].forEach(k => sessionStorage.removeItem(k));
    for (var i = 1; i <= 15; i++) sessionStorage.removeItem(photoKey(String(i)));
  } catch (_) {}
}

var LSK = { name: 'gup-ls-name', guestId: 'gup-ls-guestid', token: 'gup-ls-token' };
...
function clearIdentity() {
  try {
    [LSK.name, LSK.guestId, LSK.token].forEach(k => localStorage.removeItem(k));
  } catch (_) {}
}
...
function loadDoneSquares() {
  ...
  var params = new URLSearchParams({ guestId: confirmedGuestId, token: confirmedToken });
  fetch('/api/game/squares?' + params.toString())
    ...
    .then(function(data) {
      ...
      data.squares.forEach(function(n) {
        ...
        if (btn) { btn.classList.add('submitted'); btn.disabled = true; }
      });
      syncSubmitBtn();
    })
    .catch(function() {});
...
function markSubmitted(ns) {
  ns.forEach(function(n) {
    submittedSquares.add(n);
    squareFiles.delete(n); squareDataURLs.delete(n);
    squareTitles.delete(n); squareDescs.delete(n);
    sessionStorage.removeItem(photoKey(n));
    var btn = document.querySelector('.sq-btn[data-n="' + n + '"]');
    if (btn) { btn.classList.remove('done'); btn.classList.add('submitted'); btn.disabled = true; }
  });
  syncSubmitBtn();
  persist();
}
  ...
  try {
    ...
    if (name) {
      if (confirmed && nameConfirmed) {
        ...
        nameInput.value = name;
        if (identityModal) { identityModal.classList.add('hidden'); document.body.style.overflow = ''; }
      }
    }
