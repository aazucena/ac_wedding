...
modalConfirm.addEventListener("click", () => {
  ...
  if (pendingFile) {
    ...
    reader.onload = (e) => {
      squareDataURLs.set(n, e.target.result);
      persist();
    };
...
form.addEventListener("submit", (e) => {
  ...
  if (squareFiles.size === 0) {
    showError("Please upload at least one photo.");
    return;
  }
  var entries = [...squareFiles.entries()].sort(
  ...
  confirmDesc.textContent =
    ...
    (count !== 1 ? "s" : "") +
    " for:";
  while (confirmSquares.firstChild)
    confirmSquares.removeChild(confirmSquares.firstChild);
  entries.forEach(([n]) => {
...
confirmOk.addEventListener("click", async () => {
  ...
  for (var i = 0; i < entries.length; i += 3) {
    var batch = entries.slice(i, i + 3).map(([n, blob]) => {
      ...
      var t = squareTitles.get(n);
      if (t) data.append("title", t);
      var d = squareDescs.get(n);
      if (d) data.append("memo", d);
      return fetch("/api/game/upload", { method: "POST", body: data }).then(
        (r) => r.json(),
      );
    });
  ...
  if (failed === 0) {
    ...
    document.getElementById("success-count").textContent =
      count === 1
        ...
        : "All " + count + " photos have been received!";
    ...
    showError(
      failed +
        " of " +
        entries.length +
        " photos failed. The rest were saved. Please retry the remaining squares.",
    );
  } else {
