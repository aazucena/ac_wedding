...
export function buildGoogleUrl(e: CalendarEvent): string {
  const p = new URLSearchParams({ action: 'TEMPLATE', text: e.title, dates: `${e.start}/${e.end}` });
...
export function buildOutlookUrl(e: CalendarEvent): string {
  ...
  const p = new URLSearchParams({
    ...
    subject: e.title, startdt: fmt(e.start), enddt: fmt(e.end),
  });
  if (e.location)    p.set('location', e.location);
...
export function toCalEndDt(date: string, start?: string | null, end?: string | null): string {
