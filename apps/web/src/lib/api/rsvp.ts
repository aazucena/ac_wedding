...
export async function searchPartiesByName(name: string): Promise<Pick<Parties, 'id' | 'name' | 'rsvp_token'>[]> {
  try {
    return await get<Pick<Parties, 'id' | 'name' | 'rsvp_token'>[]>('/items/parties', {
...
export async function getGuestMessages(): Promise<Array<{ name: string; message_to_couple: string; date_rsvp_submitted?: string }>> {
  try {
...
export async function validatePartyToken(token: string): Promise<{ id: string; name: string } | null> {
  try {
 ...
 */
export async function validatePartyByIdAndToken(partyId: string, token: string): Promise<string[] | null> {
  try {
    const items = await get<{ id: string; members: { id: string }[] }[]>('/items/parties', {
...
export async function patchParty(partyId: string, payload: object): Promise<void> {
...
export async function getRsvpCount(): Promise<{ confirmed: number; total: number }> {
  try {
    const [confirmed, total] = await Promise.all([
      get<any[]>('/items/guests', { filter: { attending: { _eq: true } }, aggregate: { count: 'id' } }),
