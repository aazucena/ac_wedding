 ...
 */
export async function lookupGuestIdByName(
  name: string,
): Promise<string | null> {
  try {
 ...
 */
export async function verifyGuestNameAndTable(
  name: string,
  tableNumber: number,
): Promise<string | null> {
  try {
    const items = await get<{ id: string }[]>("/items/guests", {
      filter: {
        ...buildGuestNameFilter(name),
        table: { number: { _eq: tableNumber } },
      },
      fields: ["id"],
      limit: 1,
    });
...
export async function hasExistingGuestbookEntry(
  guestId: string,
): Promise<boolean> {
  try {
