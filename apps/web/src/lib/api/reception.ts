...
export async function getReception(): Promise<Reception> {
  try {
    ...
    }).then((data) => data[0] ?? { id: '1', start_time: '18:00:00', status: 'planning' });
  } catch {
...
const SEATING_GUEST_FIELDS = [
  ...
  'table.id', 'table.number', 'table.name', 'table.section', 'table.party.name',
];

export async function searchGuestsForSeating(nameFilter: object): Promise<any[]> {
  try {
    return await get<any[]>('/items/guests', {
      filter: { ...nameFilter, attending: { _eq: true }, table: { _nnull: true } },
...
export async function getTablemates(tableIds: string[]): Promise<any[]> {
  try {
    return await get<any[]>('/items/guests', {
      ...
      fields: ['id', 'person.first_name', 'person.last_name', 'person.preferred_name', 'table.id'],
