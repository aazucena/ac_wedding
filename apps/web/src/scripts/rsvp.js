...
function showState(state) {
  ...
  document.getElementById("state-form").style.display =
    state === "form" ? "block" : "none";
}

// ── Attending toggle ─────────────────────────────────────────────────────────
window.setAttending = function (value) {
  attending = value;
  document.getElementById("btn-yes").className = value
    ? "toggle-btn active-yes"
    : "toggle-btn";
  document.getElementById("btn-no").className = !value
...
async function handleSubmit(e) {
  ...
  const payload = {
    ...
    dietary_restrictions: attending
      ...
      : null,
    hotel_required: attending
      ? document.getElementById("hotel").checked
      : false,
    transportation_required: attending
