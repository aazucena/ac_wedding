...
function anyAttending() {
  return Object.values(guestResponse).some((v) => v === "attending");
}
function allResponded() {
  return Object.values(guestResponse).every((v) => v !== null);
}
...
function makeRadioOption(
  ...
  input.type = "radio";
  input.name = "attendance_type_" + tempId;
  input.value = value;
  if (checked) input.checked = true;
  var iconSpan = document.createElement("span");
  iconSpan.className = "card-option-icon";
  iconSpan.textContent = icon;
  var bodySpan = document.createElement("span");
  bodySpan.className = "card-option-body";
  var titleSpan = document.createElement("span");
  titleSpan.className = "card-option-title";
  titleSpan.textContent = label;
  var checkSpan = document.createElement("span");
  checkSpan.className = "card-option-check";
  checkSpan.textContent = "✓";
  bodySpan.appendChild(titleSpan);
  lbl.appendChild(input);
  lbl.appendChild(iconSpan);
  lbl.appendChild(bodySpan);
...
function createPlusOneRow(tempId: string, displayName: string) {
  ...
  removeBtn.className = "po-remove-btn";
  removeBtn.title = "Remove plus-one";
  ...
  trashSvg.setAttribute("stroke-width", "1.8");
  ...
  trashSvg.appendChild(
    svgEl("path", "d", "M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"),
  );
  trashSvg.appendChild(svgEl("path", "d", "M10 11v6M14 11v6"));
  trashSvg.appendChild(
    svgEl("path", "d", "M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"),
  );
  removeBtn.appendChild(trashSvg);
  removeBtn.addEventListener("click", function () {
    removePlusOne(tempId);
  });

  var seg = document.createElement("div");
  seg.className = "response-seg";
  seg.setAttribute("role", "group");
  seg.setAttribute("aria-label", "Response");
  var yesBtn = document.createElement("button");
  yesBtn.type = "button";
  yesBtn.className = "seg seg--yes";
  yesBtn.id = "yes-" + tempId;
  yesBtn.textContent = "✓ Attending";
  ...
  noBtn.type = "button";
  noBtn.className = "seg seg--no";
  noBtn.id = "no-" + tempId;
  noBtn.textContent = "✗ Declining";
  noBtn.addEventListener("click", function () {
    window.onResponseChange(tempId, "declined");
  });
  seg.appendChild(yesBtn);
  seg.appendChild(divider);
  seg.appendChild(noBtn);

  // Order: [trash + name] [response seg]
  nameGroup.appendChild(removeBtn);
  ...
  panel.className = "attending-panel";
  panel.id = "panel-" + tempId;
  var content = document.createElement("div");
  content.className = "attending-content";
  var inner = document.createElement("div");
  inner.className = "attending-inner";

  // Attendance choice field
  var attField = document.createElement("div");
  attField.className = "field";
  var attLabel = document.createElement("label");
  attLabel.className = "field-label";
  attLabel.textContent = "Attending";
  var optList = document.createElement("div");
  optList.className = "card-option-list";
  optList.appendChild(
    makeRadioOption(tempId, "both", "🎉", "Ceremony & Reception", true),
  );
  optList.appendChild(
    makeRadioOption(tempId, "ceremony", "⛪", "Ceremony Only", false),
  );
  optList.appendChild(
    makeRadioOption(tempId, "reception", "🥂", "Reception Only", false),
  );
  attField.appendChild(attLabel);
  attField.appendChild(optList);

  // Dietary field
  var dietField = document.createElement("div");
  dietField.className = "field";
  var dietLabel = document.createElement("label");
  dietLabel.className = "field-label";
  dietLabel.htmlFor = "dietary-" + tempId;
  ...
  dietLabel.appendChild(dietText);
  dietLabel.appendChild(optional);
  var dietArea = document.createElement("textarea");
  dietArea.id = "dietary-" + tempId;
  dietArea.rows = 2;
  dietArea.placeholder = "Allergies, intolerances, or other requirements…";

  dietField.appendChild(dietLabel);
  dietField.appendChild(dietArea);
  inner.appendChild(attField);
  inner.appendChild(dietField);
...
function showStep(step: number) {
  document
    .querySelectorAll<HTMLElement>(".wizard-step")
  ...
  if (step === 5)
    buildReview(_guestList, _plusOnes, guestResponse, anyAttending, showStep);
  document
...
window.onResponseChange = function (guestId, value) {
  ...
  if (value === "attending") {
    ...
    document
      .querySelectorAll<HTMLElement>(".attending-panel.open")
      .forEach(function (p) {
...
function initCardOptions(root: Document | HTMLElement) {
  root
    .querySelectorAll<HTMLInputElement>('.card-option input[type="radio"]')
    .forEach((radio) => {
  ...
  root
    .querySelectorAll<HTMLInputElement>('.card-option input[type="checkbox"]')
    .forEach((cb) => {
...
function refreshRepOptions() {
  ...
  document
    .querySelectorAll<HTMLElement>('[id^="rep-option-"]')
    .forEach((label) => {
      var guestId = label.id.replace("rep-option-", "");
      var visible =
        guestResponse[guestId] === "attending" &&
        label.dataset.type === "adult";
  ...
  if (checked) {
    ...
    if (checkedLabel?.style.display === "none" && target) {
      var inputA = target.querySelector<HTMLInputElement>(
        'input[type="radio"]',
      );
      if (inputA) {
        inputA.checked = true;
        document
          .querySelectorAll<HTMLElement>("#rep-list .card-option")
          .forEach((o) => o.classList.remove("selected"));
...
document.addEventListener("astro:page-load", () => {
  ...
  if (!dataEl) return;
  const { partyId, guestList, rsvpToken, plusOnesAllowed, blockedFirstNames } =
  ...
  if (_plusOnesAllowed > 0) {
    updatePlusOneUI();
    document
      .getElementById("po-add-btn")
      ?.addEventListener("click", showPlusOneForm);
    document
      .getElementById("po-cancel")
      ?.addEventListener("click", hidePlusOneForm);
    // Close on backdrop click (outside the modal panel)
    document
      .getElementById("po-modal-backdrop")
    ...
    document.addEventListener("keydown", function (e) {
      if (
        e.key === "Escape" &&
        document.getElementById("po-modal-backdrop")?.style.display === "flex"
      )
        hidePlusOneForm();
    });
    document
      .getElementById("po-confirm")
      ?.addEventListener("click", async function () {
        var firstName = (
          (document.getElementById("po-first") as HTMLInputElement | null)
            ?.value ?? ""
        ).trim();
        var lastName = (
          (document.getElementById("po-last") as HTMLInputElement | null)
            ?.value ?? ""
        ).trim();
        var gender =
          (document.getElementById("po-gender") as HTMLSelectElement | null)
            ?.value ?? "";
        var type =
          (document.getElementById("po-type") as HTMLSelectElement | null)
            ?.value ?? "adult";
        var errEl = document.getElementById("po-error");
        ...
        if (!errMsg) {
          ...
          if (
            _plusOnes.some(function (po) {
              return norm(po.firstName + " " + po.lastName) === fullNew;
            })
          ) {
            errMsg = displayName + " has already been added.";
          } else if (
            _guestList.some(function (g) {
              return norm(g.firstName + " " + g.lastName) === fullNew;
            })
          ) {
        ...
        const { error: checkError } = await actions.checkPlusOne({
          token: rsvpToken,
          partyId,
          firstName,
          lastName,
        ...
        if (confirmBtn) {
          ...
          confirmBtn.textContent = "Add Guest";
        }

        if (checkError) {
          if (errEl) {
            errEl.textContent = checkError.message;
            errEl.classList.add("visible");
          }
          return;
        }
  ...
  document.getElementById("next-1")?.addEventListener("click", () => {
    ...
    var anyMinorAttending =
      guestList.some(
        (g) => guestResponse[g.id] === "attending" && g.type !== "adult",
      ) ||
      _plusOnes.some(
        (p) => guestResponse[p.tempId] === "attending" && p.type !== "adult",
      );
    var anyAdultAttending =
      guestList.some(
        (g) => guestResponse[g.id] === "attending" && g.type === "adult",
      ) ||
      _plusOnes.some(
  ...
  document.getElementById("next-2")?.addEventListener("click", () => {
    var phone = (
      (document.getElementById("contact-phone") as HTMLInputElement | null)
        ?.value ?? ""
    ).trim();
    var email = (
    ...
    ?.addEventListener("submit", async (e) => {
      ...
      var guestPayloads = guestList.map((g) => {
        ...
        var attendanceType = isAttending
          ? (document.querySelector<HTMLInputElement>(
              `input[name="attendance_type_${g.id}"]:checked`,
            )?.value ?? "both")
          : "";
        var attendance = [
          ...(["ceremony", "both"].includes(attendanceType)
            ? ["ceremony"]
            : []),
          ...(["reception", "both"].includes(attendanceType)
            ? ["reception"]
            : []),
        ];
        return {
          ...
          dietary_restrictions: isAttending
            ? (
                document.getElementById(
                  "dietary-" + g.id,
                ) as HTMLTextAreaElement | null
      ...
      try {
        ...
        if (
          attending &&
          (!selectedGuest ||
            !selectedGuest?.personId ||
            !(contactPhone || contactEmail))
        ) {
          throw new Error(
            "RSVP failed: Guest representative is required with either the email or phone.",
          );
        }

        const representativeId =
          selectedGuest?.personId ?? guestList[0]?.personId ?? "";

        var plusOnePayloads = _plusOnes.map(function (po) {
          var isAttending = guestResponse[po.tempId] === "attending";
          var attendanceType = isAttending
            ? (document.querySelector<HTMLInputElement>(
                'input[name="attendance_type_' + po.tempId + '"]:checked',
              )?.value ?? "both")
            : "";
          var attendance = [
            ...(["ceremony", "both"].includes(attendanceType)
        ...
        const submitPayload = {
          ...
          partyId,
          partyPayload: {
            status: (attending ? "confirmed" : "declined") as
              | "confirmed"
              | "declined",
            representative: representativeId,
            hotel:
              (document.getElementById("hotel") as HTMLInputElement | null)
                ?.checked ?? false,
            transportation:
              (document.getElementById("transport") as HTMLInputElement | null)
                ?.checked ?? false,
            song_request:
              (document.getElementById("song") as HTMLInputElement | null)
                ?.value || null,
        ...
        if (plusOnePayloads.length > 0) {
          ...
          if (plusOneError)
            throw new Error(
              plusOneError.message ?? "Failed to add plus-one(s).",
            );
        }

        // Only update contact details when at least one guest is attending
        if (attending && selectedGuest?.personId) {
          const { error: contactDetailsError } =
            await actions.submitContactDetails({
              token: rsvpToken,
              partyId,
          ...
          if (contactDetailsError)
            throw new Error(contactDetailsError.message ?? "RSVP failed");
        }
        ...
        if (!anyAttending()) {
          ...
          (
            document.getElementById("success-home-btn") as HTMLElement
          ).style.display = "";
        } else {
