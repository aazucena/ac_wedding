...
export function buildReview(
  ...
  guestList.forEach(function (g) {
    ...
    badge.className =
      "review-badge " +
      (isAttending ? "review-badge--yes" : "review-badge--no");
    ...
    if (isAttending) {
      var atTypeEl = document.querySelector<HTMLInputElement>(
  ...
  plusOnes.forEach(function (po) {
    ...
    poName.textContent =
      (po.lastName ? po.firstName + " " + po.lastName : po.firstName) + " ✦";
    ...
    poHeader.appendChild(poName);
    poHeader.appendChild(poBadge);
  ...
  if (anyAttending()) {
    // ── Contact ──
    var repInput = document.querySelector<HTMLInputElement>(
      'input[name="representative"]:checked',
    );
    var repName = repInput
      ? (
          document
            .getElementById("rep-option-" + repInput.value)
            ?.querySelector(".card-option-title")?.textContent ?? ""
        ).trim()
      : "";
    var phone = (
    ...
    if (repName) {
      contactItems.appendChild(makeReviewItem("Representative", repName));
      hasContact = true;
    }
    if (phone) {
      contactItems.appendChild(makeReviewItem("Phone", phone));
      hasContact = true;
    }
    if (email) {
      contactItems.appendChild(makeReviewItem("Email", email));
      hasContact = true;
    }
    if (!hasContact)
      contactItems.appendChild(makeEmpty("No contact details provided."));
    contactSec.appendChild(contactItems);
    content.appendChild(contactSec);

    // ── Extras ──
    var hotel =
      (document.getElementById("hotel") as HTMLInputElement | null)?.checked ??
      false;
    var transport =
      (document.getElementById("transport") as HTMLInputElement | null)
        ?.checked ?? false;
    var song = (
      (document.getElementById("song") as HTMLInputElement | null)?.value ?? ""
    ).trim();
    ...
    var hasExtras = false;
    if (hotel) {
      extrasItems.appendChild(makeReviewItem("Hotel", "Requested"));
      hasExtras = true;
    }
    if (transport) {
      extrasItems.appendChild(makeReviewItem("Transport", "Requested"));
      hasExtras = true;
    }
    if (song) {
      extrasItems.appendChild(makeReviewItem("Song request", song));
      hasExtras = true;
    }
  ...
  var msgVal = (
    (document.getElementById(msgId) as HTMLTextAreaElement | null)?.value ?? ""
  ).trim();
