...
function mountCalendar(mount: HTMLElement) {
  ...
  const calEvents = (data.events as Array<Record<string, string>>).map(
    (ev) => ({
      ...
      start: Temporal.ZonedDateTime.from(
        `${(ev.start ?? "").replace(" ", "T")}:00[${tz}]`,
      ),
      end: Temporal.ZonedDateTime.from(
        `${(ev.end ?? "").replace(" ", "T")}:00[${tz}]`,
      ),
    }),
  );
  ...
  const cal = createCalendar({
    ...
    selectedDate: Temporal.PlainDate.from(
      (data.weddingDate as string).slice(0, 10),
    ),
    views: [activeView],
    events: calEvents,
    skipValidation: true,
...
function init() {
  ...
  observer?.disconnect();
  if (mqListener) {
    MOBILE_MQ.removeEventListener("change", mqListener);
    mqListener = null;
  }
  calInstance?.destroy();
  calInstance = null;
