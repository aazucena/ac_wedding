...
import { z } from 'zod';
import { validatePartyToken, uploadGuestFile, createMemoryRecord, deleteFile } from '@lib/directus';
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const caption  = rawCaption ? rawCaption.replace(/<[^>]*>/g, '').trim() || null : null;

    const parsed = uploadSchema.safeParse({ token });
    if (!parsed.success || !file) {
      return json({ error: parsed.error?.issues[0]?.message ?? 'Missing file or token.' }, 400);
    }
    ...
    try {
      await createMemoryRecord(fileId, null, caption ?? undefined, undefined, 'rsvp', false);
    } catch {
    ...
    return json({ success: true, message: 'Photo submitted! It will appear in our memories once approved.' });
