...
function toIcsTimestamp(date: string, time: string): string {
  return DateTime.fromISO(`${date}T${time}`, { zone: TZ }).toFormat(
    "yyyyMMdd'T'HHmmss",
  );
}
...
export const GET: APIRoute = async ({ url }) => {
  const parsed = eventSchema.safeParse(
    url.searchParams.get("event") ?? "ceremony",
  );
  ...
  const brunchEvent = allEvents.find(
    (e) => e.name.toLowerCase().includes("brunch") && e.date === nextDay,
  );
  ...
  > = {
    ...
    brunch: {
      ...
      dtstart: toIcsTimestamp(
        brunchEvent?.date ?? nextDay,
        brunchEvent?.start_time ?? "10:00:00",
      ),
      dtend: toIcsTimestamp(
        brunchEvent?.date ?? nextDay,
        brunchEvent?.end_time ?? "12:00:00",
      ),
