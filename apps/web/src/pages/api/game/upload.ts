...
const gameUploadSchema = z.object({
  ...
  prompt: z
    .string()
    .regex(/^\d+$/, "prompt must be a square number")
    .optional(),
});
...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    ...
    const guestToken =
      (form.get("guestToken") as string | null)?.trim() || null;
    ...
    if (!file.type.startsWith("image/"))
      return json({ ok: false, error: "Only image files are accepted." }, 400);
    ...
    const parsed = gameUploadSchema.safeParse({
      name: name ?? undefined,
    ...
    if (settings?.game_deadline) {
      ...
      const dl = DateTime.fromISO(`${weddingDate}T${settings.game_deadline}`, {
        zone: TZ,
      });
