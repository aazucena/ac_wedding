...
export const POST: APIRoute = async ({ request }) => {
  ...
  try {
    const { guestId, tableNumber } = await request.json() as { guestId?: string; tableNumber?: unknown };

    if (!guestId || !UUID_RE.test(guestId)) return json({ ok: false, error: 'Invalid request.' }, 400);
    ...
    if (!Number.isInteger(parsed) || parsed < 1) return json({ ok: false, error: 'Please enter a valid table number.' }, 400);

    const res = await fetch(
      `${DIRECTUS_URL}/items/guests/${guestId}?fields=table.number`,
      { headers: { Authorization: `Bearer ${DIRECTUS_TOKEN}` }, signal: AbortSignal.timeout(8_000) },
    );
    ...
    if (assignedTable === undefined || assignedTable === null) {
      return json({ ok: false, error: 'No table assigned — ask the MC for help.' }, 400);
    }

    if (Number(assignedTable) !== parsed) {
      return json({ ok: false, error: 'Table number doesn\'t match. Check your place card.' }, 403);
    }
