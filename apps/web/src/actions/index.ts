...
export const server = {
  ...
  submitRsvp: defineAction({
    ...
    handler: async ({ token, partyId, partyPayload, guestPayloads }) => {
      ...
      const safePayloads = guestPayloads.filter((g) =>
  ...
  checkPlusOne: defineAction({
    ...
    handler: async ({ token, partyId, firstName, lastName }) => {
      ...
      const BLOCKED: Array<[string, string, string]> = [
        [
          settings.groom?.preferred_name ?? "",
          settings.groom?.last_name ?? "Azucena",
          "the groom",
        ],
        [
          settings.groom?.first_name ?? "",
          settings.groom?.last_name ?? "Azucena",
          "the groom",
        ],
        [
          settings.bride?.preferred_name ?? "",
          settings.bride?.last_name ?? "Ranada",
          "the bride",
        ],
        [
          settings.bride?.first_name ?? "",
          settings.bride?.last_name ?? "Ranada",
          "the bride",
        ],
      ...
      const blocked = BLOCKED.find(
        ([fn, ln]) => norm(fn) === normFn && norm(ln) === normLn && fn !== "",
      );
      if (blocked)
        throw new Error(
          `${fullName} cannot be added as a plus-one (${blocked[2]}).`,
        );
      ...
      const existing = candidates.filter(
        (g) =>
          ...
          (norm(g.person.first_name) === normFn ||
            norm(g.person.preferred_name) === normFn),
      );
      ...
      if (
        existing.some(
          (g) => g.party.id !== partyId && g.party.status === "confirmed",
        )
      ) {
      ...
      throw new Error(
        `${fullName} is already registered under a different invitation.`,
      );
    },
  }),
  ...
  submitPlusOnes: defineAction({
    ...
    handler: async ({ token, partyId, plusOnePayloads }) => {
      ...
      const [partyMeta, settingsMeta] = await Promise.all([
        get<{ plus_ones_allowed: number | null }>(`/items/parties/${partyId}`, {
          fields: ["plus_ones_allowed"],
        }),
        getSettings(),
      ]);
      const cap =
        partyMeta.plus_ones_allowed ?? settingsMeta.plus_ones_allowed ?? 0;
      if (plusOnePayloads.length > cap)
        throw new Error("Plus-one limit exceeded.");

      // Block bride, groom, and their parents from being added as plus-ones.
      const groomFirst =
        settingsMeta.groom?.preferred_name ??
        settingsMeta.groom?.first_name ??
        "";
      const brideFirst =
        settingsMeta.bride?.preferred_name ??
        settingsMeta.bride?.first_name ??
        "";
      ...
      for (const p of plusOnePayloads) {
        if (BLOCKED.has(p.firstName.toLowerCase().trim())) {
          throw new Error(
            `${p.firstName} ${p.lastName} cannot be added as a plus-one.`,
          );
        }
      }
  ...
  submitContactDetails: defineAction({
    ...
    handler: async ({
      ...
      if (!memberIds.includes(guestId)) {
        throw new Error(
          "Selected representative is not a member of your party.",
        );
      }
      ...
      if (!res.ok) {
        const body = await res.json().catch(() => ({}));
        throw new Error(
          (body as any)?.error ?? "Failed to update contact details.",
        );
      }
  ...
  findSeat: defineAction({
    ...
    handler: async ({ name }) => {
      ...
      for (const m of matches) {
        (
          matchesByTable.get(m.table.id) ??
          matchesByTable.set(m.table.id, []).get(m.table.id)!
        ).push(m);
      }

      const seats = [...matchesByTable.entries()].map(
        ([tableId, tableMatches]): SeatResult => {
